% A GUI to choose different options for displaying HFD and Hurst data
function runDisplayHFDDataAllSubjects
...
% CRITICAL FIX: Syncing the 4 bands to match the extraction engine
freqRangeList0{1} = [1 90];   % Broadband (kMax=10)
freqRangeList0{2} = [8 13];   % Alpha (kMax=5)
freqRangeList0{3} = [24 34];  % Slow Gamma (kMax=5)
freqRangeList0{4} = [35 65];  % Fast Gamma (kMax=5)
...
axisRangeList0{2} = [-0.1 0.1];   axisRangeName{2} = 'YLims (Metric)'; 
axisRangeList0{3} = [-0.05 0.05]; axisRangeName{3} = 'cLims (topo)';
...
hUseMedianFlag = uicontrol('Parent',hPanel7,'Unit','Normalized','Position',[0 2/3 0.5 1/3],'Style','togglebutton','String','Median','FontSize',fontSizeMedium);
metricOptions = {'HFD', 'Hurst'};
hMetricChoice = uicontrol('Parent',hPanel7,'Unit','Normalized','BackgroundColor', backgroundColor, 'Position',[0.5 2/3 0.5 1/3],'Style','popup','String',metricOptions,'FontSize',fontSizeSmall);
...
% Generating plot handles dynamically based on the 4 bands
hAllPlots.hScatter = getPlotHandles(1,numGroups,[0.05 0.55 0.6 0.3],0.02,0.02,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plot_Callback(~,~)
        ...
        metricChoice = get(hMetricChoice,'val'); 
        ...
        % Passes metricChoice natively to the display helper
        displayHFDDataAllSubjects(subjectNameLists,protocolName,analysisChoice,refChoice,badEyeCondition,badTrialVersion,badElectrodeRejectionFlag,stRange,freqRangeList,axisRangeList,cutoffList,useMedianFlag,hAllPlots,pairedDataFlag, 1, metricChoice);
    end

    function cla_Callback(~,~)
        claGivenPlotHandle(hAllPlots.hScatter); 
    ...
    function rescale_Callback(~,~)
        ...
        rescaleGivenPlotHandle(hAllPlots.hScatter,axisLims); 
