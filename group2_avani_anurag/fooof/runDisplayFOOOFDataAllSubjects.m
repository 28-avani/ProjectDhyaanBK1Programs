% A GUI to choose different options for displaying FOOOF (1/f slope) data
function runDisplayFOOOFDataAllSubjects
...
freqRangeList0{1} = [1 150];  % Broadband
freqRangeList0{2} = [30 80];  % Gamma
...
axisRangeList0{1} = [0 150];      axisRangeName{1} = 'Freq Lims (Hz)';
axisRangeList0{2} = [0.5 2.5];    axisRangeName{2} = 'YLims (Exponent)'; 
axisRangeList0{3} = [-0.15 0.15]; axisRangeName{3} = 'cLims (topo)';
...
hAllPlots.hTopo2 = getPlotHandles(numFreqRanges,3,[0.66 0.05 0.3 0.45],0.02,0.02,1);
    ...
    function plot_Callback(~,~)
        ...
        displayFOOOFDataAllSubjects(subjectNameLists,protocolName,analysisChoice,refChoice,badEyeCondition,badTrialVersion,badElectrodeRejectionFlag,stRange,freqRangeList,axisRangeList,cutoffList,useMedianFlag,hAllPlots,pairedDataFlag, 1, metricChoice);
    end
