% =========================================================================
% FOOOF Bias Analysis: Experience, Age, Gender, Side (Protocol M1)
% =========================================================================
clear; clc;
...
protIdx = 4; % M1
freqIdx = 1; % Broadband [1 150]

% --- 1. Embed Demographics Data ---
...
medList = hoursRaw(:,1); medHours = cell2mat(hoursRaw(:,2));
...
% --- 2. Define Hemispheric Electrode Groups ---
...
globalExp = nan(numMeds, 1);
expL = nan(numMeds, numHubs); expR = nan(numMeds, numHubs);
medAge = nan(numMeds, 1); isMale = false(numMeds, 1);

% --- 3. Data Extraction ---
fprintf('Extracting FOOOF Exponents for Bias Analysis...\n');
for i = 1:numMeds
    subj = medList{i};
    fName = fullfile(fooofFolder, [subj '_ep_v8_250_1250_FOOOF.mat']);
    if exist(fName, 'file')
        tmp = load(fName, 'exponentST');
        if length(tmp.exponentST) >= protIdx && size(tmp.exponentST{protIdx}, 2) >= freqIdx
            tData = tmp.exponentST{protIdx}(:, freqIdx)';
            globalExp(i) = nanmean(tData);
            for h = 1:numHubs
                expL(i, h) = nanmean(tData(hubL{h}));
                expR(i, h) = nanmean(tData(hubR{h}));
            end
        end
    end
...
figure('Name', 'FOOOF Covariate Dashboard', 'Color', 'w', 'Position', [50 50 1250 900]);

% A. AGE BIAS
subplot(2,2,1); hold on;
v = ~isnan(medAge) & ~isnan(globalExp);
scatter(medAge(v), globalExp(v), 70, 'filled', 'MarkerFaceColor', [0.3 0.6 0.8], 'MarkerEdgeColor', 'k');
[r, p] = corr(medAge(v), globalExp(v), 'Type', 'Spearman');
plot(xlim, polyval(polyfit(medAge(v), globalExp(v), 1), xlim), 'k--');
title(sprintf('Age Bias (r = %.2f, p = %.3f)', r, p));
xlabel('Age (Years)'); ylabel('Global Exponent (E/I)'); grid on;

% B. EXPERIENCE BIAS
subplot(2,2,2); hold on;
v = ~isnan(medHours) & ~isnan(globalExp);
scatter(medHours(v), globalExp(v), 70, 'filled', 'MarkerFaceColor', [0.2 0.7 0.3], 'MarkerEdgeColor', 'k');
[r, p] = corr(medHours(v), globalExp(v), 'Type', 'Spearman');
plot(xlim, polyval(polyfit(medHours(v), globalExp(v), 1), xlim), 'k--');
title(sprintf('Experience Bias (r = %.2f, p = %.3f)', r, p));
xlabel('Practice Hours'); ylabel('Global Exponent (E/I)'); grid on;

% C. GENDER BIAS
subplot(2,2,3); hold on;
mT = globalExp(isMale & ~isnan(globalExp)); fT = globalExp(~isMale & ~isnan(globalExp));
[~, pG] = ttest2(mT, fT);
boxplot([mT; fT], [ones(size(mT)); 2*ones(size(fT))], 'Labels', {'Male','Female'});
title(sprintf('Gender Comparison (p = %.3f)', pG)); ylabel('Global Exponent'); grid on;

% D. HEMISPHERIC ASYMMETRY (SIDE BIAS)
subplot(2,2,4); hold on;
mL = nanmean(expL, 1); mR = nanmean(expR, 1);
b = bar([mL', mR'], 'EdgeColor', 'k');
b(1).FaceColor = [0.8 0.4 0.4]; b(2).FaceColor = [0.4 0.4 0.8];
xticks(1:numHubs); xticklabels(hubName);
ylabel('Hub Exponent'); title('Hemispheric Asymmetry (L vs R)');
legend({'Left', 'Right'}, 'Location', 'northeast'); grid on;

yMax = max([mL, mR]);
for h = 1:numHubs
    v = ~isnan(expL(:,h)) & ~isnan(expR(:,h));
    [~, pH] = ttest(expL(v, h), expR(v, h)); 
    star = 'n.s.'; if pH < 0.05; star = sprintf('p=%.3f *', pH); end
    text(h, yMax*1.1, star, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');
end
ylim([0 yMax*1.3]);

sgtitle('FOOOF Exponent Bias & Covariate Analysis', 'FontSize', 18, 'FontWeight', 'bold');